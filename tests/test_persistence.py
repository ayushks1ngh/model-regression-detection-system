...
async def _report() -> LocalEvaluationReport:
    """Build a completed evaluation report for persistence tests."""
...
async def test_save_and_reconstruct_run() -> None:
    engine = await _sqlite_engine()
    ...
    async with factory() as session:
        ...
        run_id = await repository.save_report("proj-1", report)
        await session.commit()

    async with factory() as session:
        loaded = await RunRepository(session).get_run(run_id)
        assert loaded is not None
        assert loaded.suite == "support-smoke"
        assert loaded.gate_outcome == report.gate.outcome.value
        assert [case.case_key for case in loaded.cases] == ["refund"]
    await dispose_engine(engine)
...
async def test_list_runs_scoped_to_project() -> None:
    ...
    async with factory() as session:
        ...
        await repository.save_report("proj-1", report)
        await session.commit()
